...
- **Performance Gains:**  
  - Achieves **61.02%** accuracy on *Alpaca*, surpassing SoTA selection and revision methods by **+2.50%** and **+2.09%**.  
  - Achieves **60.39%** on *Dolly*, outperforming prior best by **+3.65%** and **+2.60%**.
- **Sample Efficiency:**  
  - Uses **32.69% / 50.00% fewer samples** (vs. raw & best baseline) on *Alpaca*.  
  - Uses **20.00% / 79.66% fewer samples** on *Dolly*.  
- **Task-Wise Improvement:**  
  - Largest gains in **math & code reasoning** tasks (e.g., GSM8K +10.8%).  
  - Consistent improvements in **knowledge tasks** (ARC, TriviaQA, Winogrande: +1.5–2.55%).
